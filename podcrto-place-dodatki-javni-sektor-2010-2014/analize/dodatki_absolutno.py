...
def dodatki_absolutno(parser_output):
    header_cells, entries = parser_output

    entries = sorted(entries, key=structure.dodatki_sum, reverse=True)
...
if __name__ == "__main__":
    ...
    print "Top 100 lestvica po izplačilu dodatkov absolutno"
    print
    dodatki_absolutno((podatki_parsed[0][0], sum(map(lambda x: x[1], podatki_parsed), [])))
